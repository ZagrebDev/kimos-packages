...
| Versión | Cambios |
|---|---|
| **1.3.0** | 7 paletas nuevas (Sunset Coral, Rose Élégance, Ocean Blue, Purple Night, Forest Dark, Black & Gold, Grafito Mono — 13 en total). Paleta **Personalizado** con TODOS los colores definibles: fondo, acento, texto principal y secundario, superficie, bordes y color/fondo del QR, con selector "Partir desde una paleta…" para ajustar una existente. **🖌️ Aplicar este diseño a todas**: copia paleta, disposición, formato y visualización de foto/logo de la tarjeta seleccionada a todas las tarjetas (cada foto conserva su recorte), y las tarjetas nuevas —manuales o generadas desde Usuarios Kimos— heredan ese diseño automáticamente. Tool de agente `APPLY_DESIGN_TO_ALL` y soporte de `customColors` en `UPDATE_CARD`. |
