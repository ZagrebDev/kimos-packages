...
const notices = [];
const items = new Map();
const bucket = new Map();   // Cloud Storage simulado: ruta → ficha
let saved = null;
let agentReg = null;
...
const shell = {
  ...
  // Cloud Storage del tenant (APP-SPEC §7.e). La ruta la decide el host: la
  // app solo elige la carpeta lógica, así que aquí se compone igual que allí.
  files: {
    upload: async (file, opts) => {
      const folder = String((opts || {}).folder || 'documentos');
      const maxMB = Number((opts || {}).maxMB || 10);
      if (file.size > maxMB * 1024 * 1024) throw new Error('demasiado grande para el host');
      const path = 'imagenes/kreative-studio/inst-1/' + folder + '/' + file.name;
      bucket.set(path, { name: file.name, size: file.size, folder });
      return 'http://kimos.local/api/public/files/' + path;
    },
    list: async (opts) => {
      const folder = String((opts || {}).folder || '');
      return Array.from(bucket.entries())
        .filter(([, v]) => !folder || v.folder === folder)
        .map(([k, v]) => ({ url: 'http://kimos.local/api/public/files/' + k, name: v.name, size: v.size }));
    },
    remove: async (url) => {
      const path = String(url).split('/api/public/files/')[1];
      if (!bucket.has(path)) throw new Error('no existe');
      bucket.delete(path);
    },
  },
...

{
  ...
  eq('el menú expone las 22 vistas', nav.length, 22);
  const broken = [];
  ...
  check('ninguna de las 22 vistas rompe al renderizarse', broken.length === 0, broken.join(' · '));
}
...
// ── 15 ter. Archivos en el Cloud Storage del tenant (APP-SPEC §7.e) ──────
// La ruta la decide el host; la app solo elige carpeta lógica. Lo que se
// prueba aquí es el recorrido COMPLETO: el usuario pulsa el selector de la
// vista Archivos, los bytes acaban en el bucket y la ficha en la campaña.
{
  const gotoView = (label) => {
    const btn = navButtons().find((b) => {
      const l = renderDeep(b, 0).find((n) => n && n.p && n.p.className === 'ks-navitem-label');
      return l && String(l.c) === label;
    });
    if (btn) btn.p.onClick();
    return btn;
  };
  const fileInputs = () => renderDeep(app.Component(), 0)
    .filter((n) => n && n.p && n.p.type === 'file' && typeof n.p.onChange === 'function');
  const fake = (name, size, type) => ({ name, size, type: type || '' });
  const pick = async (idx, ...archivos) => {
    const inp = fileInputs()[idx];
    if (!inp) return false;
    inp.p.onChange({ target: { files: archivos, value: 'x' } });
    await sleep(20);
    return true;
  };

  check('la vista Archivos está en el menú', !!gotoView('Archivos'));
  eq('la campaña empieza sin archivos', arrayOf((await state()).files).length, 0);
  gte('la vista ofrece selectores de archivo', fileInputs().length, 4);

  // Subida real por la interfaz.
  check('se puede subir desde la vista', await pick(0, fake('brief-cliente.pdf', 120000, 'application/pdf')));
  let cat = arrayOf((await state()).files);
  eq('el archivo queda en el catálogo de la campaña', cat.length, 1);
  eq('con su carpeta lógica', cat[0].folder, 'documentos');
  eq('y su tamaño', cat[0].size, 120000);
  check('la URL es la que devolvió el host', /\/api\/public\/files\/imagenes\/kreative-studio\/inst-1\/documentos\//.test(cat[0].url), cat[0].url);
  check('los bytes están en el bucket', bucket.size === 1, bucket.size + ' en el bucket');
  const primeraRuta = Array.from(bucket.keys())[0] || '(bucket vacío)';
  check('la ruta la compuso el host, no la app',
    primeraRuta.startsWith('imagenes/kreative-studio/inst-1/'), primeraRuta);

  // El límite de tamaño de la carpeta se comprueba ANTES de molestar al host.
  const antes = bucket.size;
  notices.length = 0;
  await pick(0, fake('peliculon.mov', 80 * 1024 * 1024, 'video/quicktime'));
  eq('un archivo que excede el límite de su carpeta no se sube', bucket.size, antes);
  check('y se dice por qué', notices.some(([lvl, txt]) => lvl === 'error' && /MB/.test(txt)),
    JSON.stringify(notices.slice(0, 3)));

  // Una carpeta que solo admite imágenes rechaza lo que no lo es.
  notices.length = 0;
  const idxProducto = 1;                       // el primer uploader por carpeta
  await pick(idxProducto, fake('contrato.pdf', 2000, 'application/pdf'));
  check('la carpeta de fotos rechaza lo que no es imagen',
    notices.some(([lvl, txt]) => lvl === 'error' && /no es una imagen/.test(txt)),
    JSON.stringify(notices.slice(0, 3)));

  // Foto de producto de verdad, y reutilizarla en el brief.
  await pick(idxProducto, fake('mesa-fiordo.jpg', 340000, 'image/jpeg'));
  cat = arrayOf((await state()).files);
  const foto = cat.find((f) => f.name === 'mesa-fiordo.jpg');
  check('la foto se sube a la carpeta de producto', !!foto && foto.folder === 'producto', JSON.stringify(foto));

  // El agente ve los archivos y puede catalogar una URL ya existente.
  let sp3 = snap().archivos;
  eq('el snapshot expone el almacenamiento', sp3.almacenamiento, 'disponible (shell.files)');
  eq('y cuenta los archivos', sp3.total, cat.length);
  eq('desglosados por carpeta', sp3.porCarpeta.producto, 1);

  r = await call('ATTACH_FILE', { url: 'https://cdn.test/guion-v3.docx', name: 'Guion v3', folder: 'documentos', note: 'Aprobado por el cliente' });
  check('ATTACH_FILE cataloga una URL existente', r.success, JSON.stringify(r));
  r = await call('ATTACH_FILE', { url: 'https://cdn.test/guion-v3.docx' });
  check('ATTACH_FILE no duplica', r.success === false, JSON.stringify(r));
  r = await call('ATTACH_FILE', {});
  check('ATTACH_FILE exige la url', r.success === false);

  r = await call('LIST_FILES', {});
  check('LIST_FILES devuelve el catálogo', r.success && r.data.catalogo.length === arrayOf((await state()).files).length, JSON.stringify(r.message));
  r = await call('LIST_FILES', { folder: 'producto' });
  eq('LIST_FILES filtra por carpeta', r.data.catalogo.length, 1);

  // Contraste catálogo ↔ almacenamiento: algo que está en el bucket y no en
  // la campaña tiene que salir a la luz, no quedarse ocupando cuota en
  // silencio.
  bucket.set('imagenes/kreative-studio/inst-1/documentos/suelto.txt', { name: 'suelto.txt', size: 10, folder: 'documentos' });
  r = await call('LIST_FILES', { check: true });
  check('LIST_FILES detecta lo que sobra en el almacenamiento',
    r.data.enAlmacenamientoNoCatalogados.some((x) => x.nombre === 'suelto.txt'),
    JSON.stringify(r.data.enAlmacenamientoNoCatalogados));

  // Quitar del catálogo NO borra los bytes; borrar sí. Se separan a propósito.
  const enBucket = bucket.size;
  r = await call('REMOVE_FILE', { file: 'Guion v3' });
  check('REMOVE_FILE descataloga por nombre', r.success, JSON.stringify(r));
  eq('y no toca el almacenamiento', bucket.size, enBucket);
  r = await call('REMOVE_FILE', { file: 'no-existe.pdf' });
  check('REMOVE_FILE avisa si no lo encuentra', r.success === false && /LIST_FILES/.test(r.error), r.error);

  const pdf = arrayOf((await state()).files).find((f) => f.name === 'brief-cliente.pdf');
  r = await call('REMOVE_FILE', { file: pdf.id, wipe: true });
  check('REMOVE_FILE con wipe borra de verdad', r.success, JSON.stringify(r));
  check('los bytes ya no están en el bucket',
    !Array.from(bucket.keys()).some((k) => k.endsWith('brief-cliente.pdf')), Array.from(bucket.keys()).join(', '));
  check('y desaparece del catálogo', !arrayOf((await state()).files).some((f) => f.id === pdf.id));

  // La vista se pinta con archivos dentro, en las cuatro pieles.
  const rotoF = [];
  for (const skin of [{ form: 'classic', classicMode: 'night' }, { form: 'game', gameMode: 'kimoslab' }]) {
    await call('SET_THEME', skin);
    gotoView('Archivos');
    renderErrors = [];
    const painted = renderDeep(app.Component(), 0);
    if (painted.length < 40) rotoF.push('árbol vacío');
    for (const e of renderErrors) rotoF.push(e);
  }
  check('la vista Archivos se pinta sin romper', rotoF.length === 0, rotoF.join(' · '));
  await call('SET_THEME', { form: 'classic', classicMode: 'night' });
}
...
// app ni —mucho menos— borrar los responsables ya asignados.
{
  ...
  for (const [caso, fetchImpl] of [
    ...
    // Ese host tampoco tiene `shell.files`: Archivos debe decirlo en vez de
    // ofrecer un botón que no puede funcionar.
    try {
      const nav2 = renderDeep(a2.Component(), 0).filter((n) => n && n.p && typeof n.p.onClick === 'function'
        && typeof n.p.className === 'string' && n.p.className.indexOf('ks-navitem') >= 0);
      const bf = nav2.find((b) => renderDeep(b, 0)
        .some((n) => n && n.p && n.p.className === 'ks-navitem-label' && String(n.c) === 'Archivos'));
      if (bf) bf.p.onClick();
      renderErrors = [];
      const arb = renderDeep(a2.Component(), 0);
      const txt = [];
      const grab = (x) => { if (x == null) return; if (typeof x === 'string') { txt.push(x); return; } if (Array.isArray(x)) x.forEach(grab); };
      for (const n of arb) if (n && typeof n === 'object') grab(n.c);
      const plano = txt.join(' | ');
      check('Archivos avisa de que no hay almacenamiento ' + caso, /no ofrece almacenamiento/i.test(plano), plano.slice(0, 160));
      check('y no ofrece selector de archivo ' + caso,
        arb.filter((n) => n && n.p && n.p.type === 'file').length === 0);
      eq('nada rompió al pintarlo ' + caso, renderErrors.length, 0, renderErrors.join(' · '));
    } catch (e) { check('Archivos se pinta ' + caso, false, e.message); }
