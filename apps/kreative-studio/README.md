...
**Versión actual: 1.1.0** — la app muestra su versión en un chip de la cabecera,
para confirmar qué build sirvió la Tienda al actualizar.

| Versión | Qué trae |
|---|---|
| **1.1.0** | Archivos en el **Cloud Storage de la organización** vía `shell.files` (APP-SPEC §7.e): la ruta la decide KIMOS, con aislamiento por app, cuota atribuible y limpieza al desinstalar. Vista **Archivos** para subir, catalogar, abrir, descatalogar, borrar y contrastar el catálogo con el bucket. Tools `LIST_FILES`, `ATTACH_FILE` y `REMOVE_FILE`. La paleta de roles de escena y la tinta sobre el acento pasan a tokens con nombre, para que la app se re-marque con la marca del tenant (§9). Chip de versión en la cabecera. |
| **1.0.0** | Los doce agentes, el flujo editable con sus dos formas y siete modos, la vista Organización con responsables humanos, y el pipeline completo hasta el bundle de render. |
...
## Archivos: el Cloud Storage de la organización

Todo lo que se sube —fotos de producto, logotipos, briefs en PDF, guiones,
referencias, contratos, material entregado— va al **bucket del tenant**, y la
**ruta la decide KIMOS**, no la app: `imagenes/{appId}/{instanceId}/{carpeta}/`.
Eso es lo que da aislamiento por app, cuota atribuible y limpieza cuando alguien
desinstala. La app solo elige la **carpeta lógica**.

| Carpeta | Para qué | Límite |
|---|---|---|
| 📸 Fotos de producto | La referencia que reciben los modelos | 25 MB, solo imágenes |
| ◈ Marca | Logotipo y material de identidad | 10 MB |
| 📄 Documentos | Brief del cliente, guiones, contratos, referencias | 50 MB |
| 🎞️ Material y entregas | Lo generado y lo entregado | 200 MB |

La vista **Archivos** sube, cataloga, abre, copia la URL, reutiliza una imagen
como foto del brief, y separa dos acciones que no son la misma:

- **Quitar** lo saca del catálogo de la campaña y deja el archivo en el bucket.
- **Borrar** lo elimina del almacenamiento, y eso no se deshace.

Están separadas a propósito: una URL puede estar referenciada en el brief, en un
asset o en el script de render, y borrar los bytes dejaría un hueco silencioso.

**Comprobar almacenamiento** contrasta el catálogo con lo que hay de verdad en
el bucket y enseña lo que sobra, para que nada ocupe cuota sin que se sepa qué
es. Desde el agente: `LIST_FILES` (con `check: true`), `ATTACH_FILE` y
`REMOVE_FILE`. Subir bytes es cosa de la interfaz: el agente no maneja el
contenido binario, y fingir lo contrario sería mentir sobre lo que puede hacer.

Las URLs son de **lectura pública** —sirven en un `<img>`, en un correo o en el
script de FFmpeg—, así que ahí no va nada que no pueda serlo. Y si el host no
ofrece `shell.files`, la vista lo dice y el material se sigue pudiendo añadir
pegando su URL.
...
node apps/kreative-studio/test/test-app.mjs                # 369 comprobaciones, sin dependencias
...
node tools/pack.mjs apps/kreative-studio                   # → kreative-studio-1.1.0.kapp
```
...
(`kreative-studio-1.1.0.kapp`), como el de `productlab`. **Al subir `version` en
