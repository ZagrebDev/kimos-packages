 ...
 *   │  puertos: persistencia (saveData/items), archivos (shell.files),      │
 *   │  notificación, config, documentos · registro de tools para el AGENTE   │
 *   │  DE KIMOS (que es el orquestador externo) · 22 vistas de estudio.      │
 *   └───────────────────────────────────────────────────────────────────────┘
 *
 * ARCHIVOS: todo lo que el usuario sube —fotos, logotipos, documentos y
 * material entregado— va al Cloud Storage del tenant por `shell.files`
 * (APP-SPEC §7.e). La ruta la decide el host bajo la carpeta de la app, lo
 * que da aislamiento, cuota atribuible y limpieza al desinstalar; la app solo
 * elige la carpeta lógica y valida tamaño y tipo. La vista Archivos cataloga,
 * abre, descataloga, borra y contrasta el catálogo con el bucket.
 *
...
// Versión del build. `APP_VERSION` es el nombre que lee el revisor de la
// plataforma (APP-SPEC §7.a) y tiene que coincidir con el `version` del
// manifest; `KS_VERSION` se conserva porque ya se imprime en los scripts y
// exportaciones generados.
const APP_VERSION = '1.1.0';
const KS_VERSION = APP_VERSION;
...
/** Tamaño de archivo legible: «1,4 MB» dice más que «1468006». */
const fmtBytes = (v) => {
...
const fmtSec = (v) => {  const n = Math.max(0, num(v, 0));
...
function emptyCampaign() {
  return {
    ...
    world: wsEmptyWorld(),
    // Archivos subidos al Cloud Storage del tenant: briefs en PDF, guiones,
    // referencias, contratos, entregas… Los bytes viven en el bucket; aquí
    // solo el catálogo, para poder titularlos y saber a qué campaña sirven.
    files: [],            // [{ id, url, name, folder, mime, size, note, at }]
...
function migrate(raw) {
  ...
  out.files = arr(d.files).slice(0, FILES_MAX).map((f, i) => ({
    id: s(f.id) || 'file-' + i,
    url: s(f.url), name: s(f.name) || s(f.url).split('/').pop() || 'archivo',
    folder: s(f.folder) || 'documentos', mime: s(f.mime),
    size: num(f.size, 0), note: s(f.note), at: s(f.at) || nowIso(),
  })).filter((f) => f.url);
...
export default function mount(shell) {
  ...
  let ui = { view: 'dashboard', busy: false, sceneSel: null, formatSel: null, promptCap: 'video',
    ...
    worldTab: 'areas', worldArea: null, worldStaff: null, fileFolder: 'all' };
  ...
  // ── Puerto de archivos · Cloud Storage del tenant (APP-SPEC §7.e) ──────
  //
  // La ruta real NO la decide la app: la decide el host, bajo
  // `imagenes/{appId}/{instanceId}/{carpeta}/`. Eso da aislamiento por app,
  // cuota atribuible al tenant y limpieza al desinstalar — tres cosas que una
  // ruta inventada por la app no puede dar. Antes esto subía a mano contra
  // `/api/v2/files`; ahora pasa por `shell.files`.
  const hasFiles = !!(shell.files && typeof shell.files.upload === 'function');
  const NO_FILES = 'Este host no ofrece almacenamiento (shell.files). Puedes seguir añadiendo material por URL.';

  /** Carpetas lógicas de la app. Una por tipo de material, no una por campaña. */
  const FILE_FOLDERS = [
    { id: 'producto', label: 'Fotos de producto', emoji: '📸', maxMB: 25, accept: 'image/*' },
    { id: 'marca', label: 'Marca', emoji: '◈', maxMB: 10, accept: 'image/png,image/svg+xml,image/webp,image/jpeg' },
    { id: 'documentos', label: 'Documentos', emoji: '📄', maxMB: 50, accept: '' },
    { id: 'entregas', label: 'Material y entregas', emoji: '🎞️', maxMB: 200, accept: '' },
  ];
  const folderById = (id) => FILE_FOLDERS.find((f) => f.id === s(id)) || FILE_FOLDERS[2];

  /**
   * Sube un archivo al almacenamiento del tenant y devuelve su URL pública.
   *
   * El límite de tamaño lo pone la carpeta y lo comprueba también el host; el
   * tipo lo valida la app, porque la plataforma no puede adivinar qué es
   * aceptable aquí.
   */
  async function uploadFile(file, folder) {
    if (!hasFiles) throw new Error(NO_FILES);
    const f = folderById(folder);
    const size = num(obj(file).size, 0);
    if (size > f.maxMB * 1024 * 1024) {
      throw new Error('«' + s(file.name) + '» pesa ' + fmtBytes(size)
        + ' y el máximo de ' + f.label.toLowerCase() + ' es ' + f.maxMB + ' MB.');
    }
    if (f.accept === 'image/*' && !/^image\//.test(s(obj(file).type))) {
      throw new Error('«' + s(file.name) + '» no es una imagen.');
    }
    const url = await shell.files.upload(file, { folder: f.id, maxMB: f.maxMB });
    const out = s(typeof url === 'string' ? url : obj(url).url);
    if (!out) throw new Error('El host no devolvió la URL del archivo subido.');
    return out;
  }

  /** Lo que hay de verdad en una carpeta del almacenamiento. */
  async function listFiles(folder) {
    if (!hasFiles || typeof shell.files.list !== 'function') return null;
    try {
      const raw = await shell.files.list({ folder: folderById(folder).id });
      // El host puede devolver cadenas o fichas; se aceptan las dos formas.
      return arr(raw).map((x) => (typeof x === 'string'
        ? { url: x, name: s(x).split('/').pop() }
        : { url: s(obj(x).url || obj(x).publicUrl), name: s(obj(x).name || obj(x).filename),
          size: num(obj(x).size, 0), at: s(obj(x).createdAt || obj(x).at) }))
        .filter((x) => x.url);
    } catch (e) { return null; }
  }

  /** Borra del almacenamiento. Solo puede borrar dentro del espacio de la app. */
  async function removeStoredFile(url) {
    if (!hasFiles || typeof shell.files.remove !== 'function') return false;
    try { await shell.files.remove(s(url)); return true; } catch (e) { return false; }
  }

  /**
   * Sube y cataloga: los bytes quedan en el bucket del tenant y la ficha en la
   * campaña. Sin la ficha, el archivo existiría pero nadie sabría qué es.
   */
  async function uploadAndRegister(file, folder, note) {
    const url = await uploadFile(file, folder);
    const rec = {
      id: newId('file'), url, name: s(obj(file).name) || 'archivo',
      folder: folderById(folder).id, mime: s(obj(file).type),
      size: num(obj(file).size, 0), note: s(note), at: nowIso(),
    };
    patch((m) => {
      m.files = arr(m.files).concat([rec]).slice(-FILES_MAX);
      logLine(m, 'success', 'Archivo «' + rec.name + '» subido a ' + folderById(folder).label + '.');
    });
    return rec;
  }

  /**
   * Quita un archivo del catálogo y, si se pide, del almacenamiento.
   *
   * Se separan a propósito: una URL puede estar referenciada en el brief o en
   * un asset, y borrar los bytes dejaría un hueco silencioso. Quien borra de
   * verdad lo hace a sabiendas.
   */
  async function dropFile(id, alsoStorage) {
    const rec = arr(model.files).find((f) => f.id === s(id) || f.url === s(id));
    if (!rec) { notify('error', 'Ese archivo no está en el catálogo.'); return false; }
    let wiped = false;
    if (alsoStorage) wiped = await removeStoredFile(rec.url);
    patch((m) => {
      m.files = arr(m.files).filter((f) => f.id !== rec.id);
      logLine(m, 'info', 'Archivo «' + rec.name + '» '
        + (wiped ? 'borrado del almacenamiento.' : 'quitado del catálogo (sigue en el almacenamiento).'));
    });
    if (alsoStorage && !wiped) notify('warn', 'Se quitó del catálogo, pero el host no pudo borrarlo del almacenamiento.');
    else notify('success', 'Archivo «' + rec.name + '» ' + (wiped ? 'borrado.' : 'quitado del catálogo.'));
    return true;
  }
  ...
  const AGENT_TOOLS = [
    ...
    { name: 'LIST_FILES',
      description: 'Lista los archivos de la campaña guardados en el Cloud Storage de la organización: fotos de producto, marca, documentos y material entregado. Con `check: true` contrasta además el catálogo con lo que hay de verdad en el almacenamiento y señala lo que sobra.',
      inputSchema: { type: 'object', properties: {
        folder: { type: 'string', description: 'producto | marca | documentos | entregas; vacío = todas.' },
        check: { type: 'boolean', description: 'Contrastar con el almacenamiento real.' },
      } } },
    { name: 'ATTACH_FILE',
      description: 'Añade al catálogo de archivos de la campaña una URL que ya existe (por ejemplo, algo que acabas de generar y registrar). NO sube bytes: subir un archivo del disco se hace desde la vista Archivos, porque el agente no maneja el contenido binario.',
      inputSchema: { type: 'object', properties: {
        url: { type: 'string' },
        name: { type: 'string', description: 'Cómo se llama. Vacío = el último tramo de la URL.' },
        folder: { type: 'string', description: 'producto | marca | documentos | entregas.' },
        note: { type: 'string', description: 'Para qué sirve.' },
      }, required: ['url'] } },
    { name: 'REMOVE_FILE',
      description: 'Quita un archivo del catálogo de la campaña. Con `wipe: true` lo borra también del almacenamiento, que no se puede deshacer; por defecto solo lo descataloga, porque su URL puede estar referenciada en el brief o en un asset.',
      inputSchema: { type: 'object', properties: {
        file: { type: 'string', description: 'Id, nombre o URL del archivo.' },
        wipe: { type: 'boolean', description: 'Borrar también del almacenamiento.' },
      }, required: ['file'] } },
    { name: 'RUN_AGENT',
  ...
  async function dispatch(action) {
    ...
    try {
      ...
      if (type === 'LIST_FILES') {
        const want = s(p.folder) ? folderById(p.folder).id : '';
        const cat = arr(model.files).filter((f) => !want || f.folder === want);
        const data = { catalogo: cat.map((f) => ({ id: f.id, nombre: f.name, carpeta: f.folder,
          url: f.url, tamano: f.size, nota: f.note, fecha: f.at })) };
        let extra = '';
        if (p.check) {
          if (!hasFiles) extra = ' Este host no ofrece almacenamiento, así que no hay nada que contrastar.';
          else {
            const known = new Set(cat.map((f) => f.url));
            const huerfanos = [];
            let leidas = 0;
            for (const f of FILE_FOLDERS) {
              ...
              leidas++;
              for (const x of got) if (!known.has(x.url)) huerfanos.push({ carpeta: f.id, url: x.url, nombre: x.name });
            }
            data.enAlmacenamientoNoCatalogados = huerfanos;
            extra = leidas
              ? ' ' + huerfanos.length + ' archivo(s) están en el almacenamiento y no en el catálogo.'
              : ' El host no deja listar las carpetas, así que no se pudo contrastar.';
          }
        }
        return { success: true,
          message: cat.length + ' archivo(s) en el catálogo' + (want ? ' de ' + folderById(want).label : '') + '.' + extra,
          data };
      }

      if (type === 'ATTACH_FILE') {
        const url = resolveUrl(p.url);
        if (!url) return { success: false, error: 'Indica la `url` del archivo.' };
        if (arr(model.files).some((f) => f.url === url)) {
          return { success: false, error: 'Ese archivo ya está en el catálogo.' };
        }
        const f = folderById(p.folder);
        const rec = { id: newId('file'), url, name: s(p.name) || s(url).split('/').pop() || 'archivo',
          folder: f.id, mime: '', size: 0, note: s(p.note), at: nowIso() };
        patch((m) => {
          m.files = arr(m.files).concat([rec]).slice(-FILES_MAX);
          logLine(m, 'info', 'Archivo «' + rec.name + '» catalogado en ' + f.label + '.');
        });
        return { success: true, message: '«' + rec.name + '» catalogado en ' + f.label + '.',
          data: { id: rec.id, url: rec.url } };
      }

      if (type === 'REMOVE_FILE') {
        const needle = s(p.file);
        const rec = arr(model.files).find((f) => f.id === needle || f.url === needle || norm(f.name) === norm(needle));
        if (!rec) {
          return { success: false, error: 'No hay ningún archivo que case con «' + needle + '». '
            + 'Usa LIST_FILES para ver el catálogo.' };
        }
        const okDrop = await dropFile(rec.id, !!p.wipe);
        if (!okDrop) return { success: false, error: 'No se pudo quitar el archivo.' };
        return { success: true, message: '«' + rec.name + '» ' + (p.wipe ? 'borrado del almacenamiento.' : 'quitado del catálogo.') };
      }
    ...
    try {
      unregisterAgent = shell.agent.register({
        ...
        getSnapshot: () => ({
          ...
          archivos: {
            almacenamiento: hasFiles ? 'disponible (shell.files)' : 'no disponible en este host',
            total: arr(model.files).length,
            porCarpeta: FILE_FOLDERS.reduce((acc, f) => {
              acc[f.id] = arr(model.files).filter((x) => x.folder === f.id).length; return acc;
            }, {}),
            lista: arr(model.files).slice(-40).map((f) => ({ id: f.id, nombre: f.name,
              carpeta: f.folder, url: f.url, nota: f.note })),
          },
          organizacion: {
  ...
  const VIEWS = [
    ...
    { id: 'files', label: 'Archivos', emoji: '🗄', group: 'Distribución' },
  ...
  function AssetsView() {
    ...
    async function onFiles(files) {
      ...
      for (const f of fs2) {
        try {
          const url = await uploadFile(f, 'entregas');
  ...
  const GUIDE_DELIVERS = [
    ...
    { view: 'files', icon: '🗄', title: 'Archivos', text: 'Fotos, logotipos, briefs en PDF, guiones y material entregado, guardados en el Cloud Storage de la organización. La ruta la decide KIMOS: aislamiento por app, cuota atribuible y limpieza al desinstalar.' },
  ...
  const VIEW_COMPONENTS = {
    ...
    assets: AssetsView, files: FilesView, analytics: AnalyticsView, styles: StylesView, versions: VersionsView,
    ...
    return h('div', {
      ...
      h('header', { className: 'ks-top', key: 't' }, [
        ...
        h('div', { className: 'ks-top-meta', key: 'm' }, [
          ...
          // Qué build tomó el host: sin esto, al probar una actualización no
          // hay forma de saber si la Tienda sirvió la nueva o la de antes.
          h(Chip, { key: 'v', title: 'Versión instalada de Kreative Studio' }, 'v' + APP_VERSION),
        ]),
