...
| `LIST_FILES` | Lista los archivos de la campaña en el Cloud Storage de la organización. Con `check: true` contrasta el catálogo con lo que hay de verdad en el bucket y señala lo que sobra. |
| `ATTACH_FILE` | Cataloga una URL que ya existe. **No sube bytes**: eso se hace desde la vista Archivos, porque el agente no maneja contenido binario. |
| `REMOVE_FILE` | Quita del catálogo; con `wipe: true` borra también del almacenamiento, que no se deshace. |
