/**
 * AIEP GESTIÓN — asistencia del Seminario «IA y Protección de Datos»,
 * Sede AIEP San Joaquín.
 *
 * QUÉ HACE: es el lado interno del par. Los totem de AIEP INSCRIPCIÓN acreditan
 * gente en la puerta; aquí se ve quién llegó, quién falta, se busca por nombre
...
const normalizarRut = (b) => String(b || '').toUpperCase().replace(/[^0-9K]/g, '');
...
const centroDe = (p) => (p && String(p.c || p.cdn || '').trim()) || SIN_CENTRO;
...
/* ── Padrón de inscritos ───────────────────────────────────────────────
   Copia literal del padrón de AIEP INSCRIPCIÓN.
...
/**
 * Cruza el padrón con lo que llegó. Una persona puede aparecer varias veces
 * (se acreditó dos veces, o entró por dos totem): se queda la PRIMERA marca,
 * que es la hora real de llegada, y se cuentan las repeticiones aparte.
 */
...
export default function mount(shell) {
  ...
  /** Sanea el documento propio (disco, agente o restauración de versión). */
  function normalizarDoc(bruto) {
    const d = bruto && typeof bruto === 'object' ? bruto : {};
    const manuales = {};
    Object.keys(d.manuales || {}).slice(0, 5000).forEach((k) => {
      const rut = normalizarRut(k);
      const m = d.manuales[k];
      if (!rut || !m) return;
      manuales[rut] = {
  ...
  const commitDoc = (parcial) => {
    doc = normalizarDoc(Object.assign({}, doc, parcial));
    emitir();
    programarGuardado();
  };
  ...
  const programarRefresco = () => {
    ...
    refrescoT = setTimeout(() => { cargar().then(programarRefresco); }, REFRESCO_MS);
  };
  ...
  function estado() {
    ...
    conManual.forEach((f) => {
      const c = f.centro;
      if (!centros[c]) centros[c] = { total: 0, llegaron: 0 };
      centros[c].total += 1;
      if (f.llego) centros[c].llegaron += 1;
    });
    return {
      filas: conManual,
  ...
  const irA = (seccion) => {
    ...
    setVista({ seccion, destacado: null });
    return true;
  };

  /** Marca o desmarca a mano. Reversible y con nota: nada se borra. */
  function marcarManual(rutBruto, nota) {
    const rut = normalizarRut(rutBruto);
    if (!rut) return { ok: false, error: 'RUT vacío.' };
    const manuales = Object.assign({}, doc.manuales);
    if (manuales[rut]) {
      delete manuales[rut];
      commitDoc({ manuales });
      return { ok: true, marcado: false, rut };
    }
    manuales[rut] = { ts: Date.now(), nota: String(nota || 'Marcado a mano en gestión').slice(0, 200) };
    commitDoc({ manuales });
  ...
  Promise.resolve(shell.loadData ? shell.loadData() : null)
    .then((d) => { if (d) { doc = normalizarDoc(d); emitir(); } })
    .catch(() => {});

  if (shell.window && shell.window.setTitle) shell.window.setTitle('AIEP GESTIÓN · ' + EVENTO.titulo);
  if (shell.documents) {
  ...
  if (shell.agent && shell.agent.register) {
    offAgent = shell.agent.register({
      label: 'AIEP GESTIÓN · Asistencia',
      description: 'Asistencia del Seminario «IA y Protección de Datos» en la Sede AIEP San '
        + 'Joaquín: quién llegó, quién falta, reparto por Centro de Negocios SERCOTEC y '
        + 'marcado manual de quien se acreditó sin pasar por el totem.',
      tools: [
        {
          name: 'IR_A_SECCION',
          description: 'Muestra una sección del panel.',
          inputSchema: {
            type: 'object',
            properties: { seccion: { type: 'string', enum: IDS_SECCION } },
            required: ['seccion'],
          },
        },
        {
          name: 'RESUMEN',
          description: 'Cuántos llegaron, cuántos faltan y cómo va cada Centro de Negocios. '
            + 'No modifica nada.',
          inputSchema: { type: 'object', properties: {} },
        },
        {
          name: 'BUSCAR',
          description: 'Busca personas por nombre, empresa o RUT y deja el resultado filtrado '
            + 'EN PANTALLA.',
          inputSchema: {
            type: 'object',
        ...
        {
          name: 'FILTRAR',
          description: 'Filtra la lista por Centro de Negocios y/o por quienes aún no llegan, '
            + 'y deja la vista aplicada en pantalla.',
          inputSchema: {
            type: 'object',
            properties: {
              centro: { type: 'string', description: 'Nombre del Centro de Negocios, o vacío para todos.' },
              soloFaltan: { type: 'boolean' },
            },
          },
        },
        {
          name: 'MARCAR_ASISTENCIA',
          description: 'Marca a mano que una persona asistió, cuando se acreditó sin pasar por '
            + 'el totem. Si ya estaba marcada a mano, la desmarca. Es reversible y queda con '
            + 'nota; NO borra ni edita lo que registró el totem.',
          inputSchema: {
            type: 'object',
            properties: {
              rut: { type: 'string' },
              nota: { type: 'string', description: 'Por qué se marca a mano (queda guardado).' },
            },
            required: ['rut'],
          },
        },
      ...
      getSnapshot: () => {
        ...
        return {
          ...
          enPantalla: {
            ...
            busqueda: vista.busqueda || null,
            centro: vista.centro || null,
            soloFaltan: vista.soloFaltan,
          },
          avisos: [
            'Aquí hay datos personales de asistentes: RUT, correos y empresas. No los recites '
              + 'en voz alta ni los saques de la pantalla salvo que te lo pidan explícitamente '
      ...
      dispatchAction: async (action) => {
        ...
        try {
          ...
          if (tipo === 'RESUMEN') {
            ...
            return {
              success: true,
              message: e.llegaron + ' de ' + e.inscritos + ' inscritos ya se acreditaron.',
              data: {
                inscritos: e.inscritos,
                llegaron: e.llegaron,
                faltan: Math.max(0, e.inscritos - e.llegaron),
          ...
          if (tipo === 'BUSCAR') {
            const q = String(p.texto || '').trim();
            if (q.length < 2) return { success: false, error: 'Escribe al menos 2 caracteres.' };
            setVista({ seccion: 'asistencia', busqueda: q });
            const hits = filtrar(estado().filas, { busqueda: q, centro: '', soloFaltan: false });
            return {
              success: true,
              message: hits.length
                ? hits.length + ' persona' + (hits.length === 1 ? '' : 's') + ' en pantalla.'
          ...
          if (tipo === 'FILTRAR') {
            ...
            const e = estado();
            if (centro && !Object.keys(e.centros).includes(centro)) {
              return { success: false, error: 'No hay un centro llamado "' + centro + '". Los que hay: ' + Object.keys(e.centros).join(', ') };
            }
            setVista({ seccion: 'asistencia', centro, soloFaltan: p.soloFaltan === true, busqueda: '' });
            const hits = filtrar(e.filas, { busqueda: '', centro, soloFaltan: p.soloFaltan === true });
            return {
              success: true,
              message: hits.length + ' persona(s) en pantalla'
                + (centro ? ' de ' + centro : '') + (p.soloFaltan ? ', solo quienes faltan' : '') + '.',
              data: { total: hits.length, centro: centro || 'todos', soloFaltan: p.soloFaltan === true },
            };
          }
          if (tipo === 'MARCAR_ASISTENCIA') {
            const rut = normalizarRut(p.rut);
            if (!rutValido(rut)) return { success: false, error: 'Ese RUT no es válido: ' + p.rut };
            const persona = estado().filas.find((f) => f.rut === rut);
            if (persona && persona.llego && !persona.manual) {
              return {
                success: false,
                error: 'Esa persona ya la registró el totem a las ' + horaDe(persona.hora)
                  + '. No hace falta marcarla a mano.',
              };
            }
            const r = marcarManual(rut, p.nota);
            if (!r.ok) return { success: false, error: r.error };
            setVista({ seccion: 'asistencia', destacado: rut });
            return {
              success: true,
              message: r.marcado
  ...
  /* ── Filtro y orden (compartidos por la UI y el agente) ────────────── */

  function filtrar(filas, f) {
    const q = plano(f.busqueda).trim();
    const qRut = normalizarRut(f.busqueda);
    return filas.filter((x) => {
      if (f.centro && x.centro !== f.centro) return false;
      if (f.soloFaltan && x.llego) return false;
      if (!q) return true;
      if (qRut.length >= 4 && x.rut.includes(qRut)) return true;
      return plano(x.nombre).includes(q) || plano(x.empresa).includes(q);
    });
  }

  function ordenar(filas, orden) {
    const copia = filas.slice();
    if (orden === 'nombre') return copia.sort((a, b) => plano(a.nombre).localeCompare(plano(b.nombre)));
    if (orden === 'centro') {
      return copia.sort((a, b) => a.centro.localeCompare(b.centro) || plano(a.nombre).localeCompare(plano(b.nombre)));
    }
    // Por hora: primero quienes llegaron, en orden de llegada; luego el resto.
    return copia.sort((a, b) => {
      if (a.llego !== b.llego) return a.llego ? -1 : 1;
      if (a.llego) return String(a.hora).localeCompare(String(b.hora));
      return plano(a.nombre).localeCompare(plano(b.nombre));
    });
  }
      ...
      h('div', { className: 'ge-kpis' },
        ...
        h('div', { className: 'ge-kpi' },
          ...
          h('p', { className: 'ge-kpi-x' }, e.repeticiones
            ...
            : 'Sin repeticiones'))),
      h('div', { className: 'ge-avance', role: 'img', 'aria-label': pct + '% de los inscritos ya llegó' },
        h('div', { className: 'ge-avance-f', style: { width: pct + '%' } })));
  }
  ...
  function Tabla(props) {
    ...
    const col = (id, label) => h('th', {
      scope: 'col',
      onClick: () => setVista({ orden: id }),
      title: 'Ordenar por ' + label.toLowerCase(),
    }, label, props.orden === id ? ' ▾' : '');
    return h('table', { className: 'ge-tabla' },
      h('thead', null, h('tr', null,
        col('nombre', 'Nombre'),
        h('th', { scope: 'col' }, 'RUT'),
        h('th', { scope: 'col' }, 'Empresa'),
        col('centro', 'Centro'),
        col('hora', 'Asistencia'),
        h('th', { scope: 'col' }, ''))),
      ...
      h('td', null, f.llego
        ? h('span', { className: 'ge-marca si' }, '✓ ', horaDe(f.hora), f.manual ? ' · a mano' : '')
        : h('span', { className: 'ge-marca no' }, 'No llegó')),
      h('td', null, f.llego && !f.manual
        ? null
        : h('button', {
          type: 'button', className: 'ge-mini',
          onClick: () => marcarManual(f.rut, f.manual ? '' : 'Marcado a mano en gestión'),
        }, f.manual ? 'Quitar marca' : 'Marcar a mano'))))));
  }

  function Asistencia(props) {
    const e = props.e;
    const v = props.vista;
    const filas = ordenar(filtrar(e.filas, v), v.orden);
  ...
  function Centros(props) {
    const e = props.e;
    const nombres = Object.keys(e.centros).sort();
    return h('div', { className: 'ai-wrap' },
      h('div', { className: 'ai-card' },
        h('p', { className: 'ai-h3 rojo' }, 'Centros de Negocios'),
      ...
      nombres.map((n) => {
        ...
        return h('div', { className: 'ai-card', key: n },
          h('h3', { className: 'ai-h2', style: { fontSize: 'var(--f-lg)' } }, n),
          h('p', { className: 'ai-p' },
            h('strong', null, String(c.llegaron)), ' de ', String(c.total), ' · ', String(pct), '%'),
          h('div', { className: 'ge-avance' }, h('div', { className: 'ge-avance-f', style: { width: pct + '%' } })),
          h('div', { style: { marginTop: '.9em' } },
            h('button', {
              type: 'button', className: 'ai-btn ghost',
              onClick: () => setVista({ seccion: 'asistencia', centro: n, busqueda: '', soloFaltan: false }),
            }, 'Ver sus ' + c.total + ' personas'),
            h('button', {
              type: 'button', className: 'ai-btn ghost', style: { marginLeft: '8px' },
              onClick: () => setVista({ seccion: 'asistencia', centro: n, busqueda: '', soloFaltan: true }),
            }, 'Ver quiénes faltan')));
      }));
  }
  ...
  function Component() {
    ...
    React.useEffect(() => {
      listeners.add(setEst);
      return () => { listeners.delete(setEst); };
    }, []);
    ...
    /* La descarga se arma en el navegador: el CSV no sale de la máquina de
       quien lo pide, y no hace falta un endpoint para exportar. */
    const exportar = () => {
      const filas = ordenar(filtrar(e.filas, v), v.orden);
      const texto = '﻿' + csv(filas);   // BOM: Excel en es-CL abre UTF-8
      try {
        const blob = new Blob([texto], { type: 'text/csv;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'asistencia-seminario-aiep-' + EVENTO.anio + EVENTO.dia + '.csv';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        setTimeout(() => URL.revokeObjectURL(url), 1000);
        shell.notify && shell.notify({ level: 'success', text: filas.length + ' filas exportadas.' });
      } catch (err) {
        shell.notify && shell.notify({ level: 'error', text: 'No se pudo generar el CSV.' });
      }
    };
    ...
    return h('div', { ref: raizRef, className: 'kimos-aiep-gestion modo-' + modo },
      ...
      activa === 'asistencia' ? h('div', { className: 'ge-barra' },
        h('input', {
          className: 'ge-buscar', type: 'search', value: v.busqueda,
          placeholder: 'Buscar por nombre, empresa o RUT…',
          onChange: (ev) => setVista({ busqueda: ev.target.value }),
        }),
        h('div', { className: 'ge-filtros' },
          centros.map((c) => h('button', {
            key: c || 'todos', type: 'button',
            className: 'ge-f' + (v.centro === c ? ' on' : ''),
            onClick: () => setVista({ centro: c }),
          }, c || 'Todos')),
          h('button', {
            type: 'button', className: 'ge-f' + (v.soloFaltan ? ' on' : ''),
            onClick: () => setVista({ soloFaltan: !v.soloFaltan }),
          }, 'Solo quienes faltan'))) : null,

      h('main', { className: 'ai-body' }, (secciones[activa] || secciones.asistencia)()),
