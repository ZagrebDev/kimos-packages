# Pack Triángulo Pickler — del personalizador a ProductLab

Mueble infantil de **terciado de abedul** (86 cm de lado mayor, medido del GLB).
Partes tintables: **Laterales**, **Tablas**, **Estructura** · 6 acabados.
...
`public/models/triangulo_pickler.glb` y `public/textures/plywood_col.webp` (una sola vez, sirve para todos).

## Pasos del demo

1. **Crear el producto** `Triángulo Pickler` en ProductLab (pestaña Productos → + Añadir).
2. **Pestaña Visor 3D** → activar → **Subir .glb** (`triangulo_pickler.glb`). Copia la URL.
