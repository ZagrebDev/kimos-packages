...
`.kapp` entero —bundle y motor— y es la vía recomendada para esta app.

Por la vía del catálogo oficial el host descarga el bundle y **solo los assets
declarados** en `assets[]` del manifest, que los sirve en
`/api/apps/{id}/asset/{ruta}` para `shell.assetUrl` (backend de kimos-enterprice,
`appsAPI.py`). Por eso el manifest declara los cinco archivos del motor: sin esa
lista, la pestaña 🔌 Puente no podría entregarlos en una instalación desde la
Tienda. Ojo con una diferencia que sigue en pie: por esa vía el `.kapp` no se
publica, así que `install_vps.sh` —que espera `requirements.txt` y
`geminis.service` a su lado— necesita que se descarguen los cinco archivos al
mismo directorio del VPS.
